...
Be friendly, engaging, and helpful. Keep responses conversational and warm."""
    ...
    try:
        print(f"🤖 {agent_info['name']}: Calling Ollama...")
        ...
        print(f"❌ {agent_info['name']}: Error calling Ollama: {e}")
...
You are part of a multi-agent team responding to a complex request. Provide your specialized perspective on this query. Be concise but thorough in your area of expertise."""
        ...
        try:
            response = model.invoke(full_prompt)
            agent_outputs[agent_id] = response
            all_responses.append(f"**{agent_info['name']}:** {response}")
...
Please provide a cohesive, well-structured final response that synthesizes the different perspectives while maintaining clarity and avoiding redundancy. Make sure the response flows naturally and addresses the user's needs comprehensively."""
    ...
    try:
        # Combine coordination prompt and content for Ollama
        final_prompt = f"{coordination_prompt}\n\nUser query: {last_message}\n\nAgent responses:\n{combined_response}\n\nCoordinated response:"
        ...
        print(f"   • Final length: {len(final_response)} characters")
        print(f"   • Compression ratio: {len(final_response) / len(combined_response) * 100:.1f}%")
        print("=" * 80)
        
        # Create a proper message object
        final_response_message = HumanMessage(content=final_response)
        
        return Command(
            goto=END,
            update={"messages": [final_response_message]},
        )
    except Exception as e:
        print(f"❌ MULTI-AGENT: Error in final coordination: {e}")
        print("🔄 Falling back to combined responses...")
        # Fallback to combined responses
        fallback_response = HumanMessage(content=combined_response)
        return Command(
            goto=END,
